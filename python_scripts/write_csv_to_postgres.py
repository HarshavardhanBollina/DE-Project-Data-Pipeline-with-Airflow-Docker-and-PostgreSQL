...
postgres_database = 'CSVData_Airflow_postgres_docker'
postgres_user = 'harsha'
postgres_password = 'v@rdhan97'
postgres_port = 5432
dest_folder = os.environ.get('dest_folder')

url = "https://github.com/HarshavardhanBollina/DE-Project-Data-Pipeline-with-Airflow-Docker-and-PostgreSQL/blob/main/Source_CSV_Files/churn_modelling.csv"
...
def create_postgres_table():
    ...
    """
    try:
        cur.execute("""CREATE TABLE IF NOT EXISTS postgres_database.csv_schema.churn_modelling (RowNumber INTEGER PRIMARY KEY, CustomerId INTEGER, 
...
def write_to_postgres():
    ...
    """
    df = pd.read_csv(f'{dest_folder}/churn_modelling.csv', sep=';')
    inserted_row_count = 0

    for _, row in df.iterrows():
        count_query = f"""SELECT COUNT(*) FROM postgres_database.csv_schema.churn_modelling WHERE RowNumber = {row['RowNumber']}"""
        cur.execute(count_query)
        result = cur.fetchone()
        
        if result[0] == 0:
            inserted_row_count += 1
            cur.execute("""INSERT INTO postgres_database.csv_schema.churn_modelling (RowNumber, CustomerId, Surname, CreditScore, Geography, Gender, Age, 
